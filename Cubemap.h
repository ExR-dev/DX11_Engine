...
public:
	Cubemap();
	Cubemap(ID3D11Device *device, UINT resolution, float nearZ, float farZ, const XMFLOAT4A &initialPosition);
	~Cubemap();
	...
	[[nodiscard]] bool Initialize(ID3D11Device *device, UINT resolution, float nearZ, float farZ, const XMFLOAT4A &initialPosition);
